...
// 1.4. 绑定连接完成的监听(用来提示连接成功)
conn.on('/register',function () { // 连接成功回调
    console.log('服务器连接成功啦！YE！！！')
})
...
function testFind() {
    ...
    UserModel.find({_id:'5ae40cbb2d9f713cf4143b3c'}, function (error, users) {
        console.log('find()', error, users)
    })
    // 查询一个: 得到是匹配的文档对象, 如果没有匹配的就是null
    UserModel.findOne({_id:'5ae40cbb2d9f713cf4143b3c'}, function (error, user) {
...
// 3.3. 通过Model的findByIdAndUpdate()更新某个数据
function testUpdate() {
    UserModel.findByIdAndUpdate({_id:'5ae40cbb2d9f713cf4143b3c'},
...
// 3.4. 通过Model的remove()删除匹配的数据
function testDelete() {
    UserModel.remove({_id:'5ae40cbb2d9f713cf4143b3c'}, function (error, doc) {
